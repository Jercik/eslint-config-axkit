...
| `vitest`        | `boolean` | Enable Vitest ESLint rules for test files (default: `true`). Set to `false` when using `node:test` or another runner — this also relaxes `@typescript-eslint/no-floating-promises` in test files.                                                                |
