...
| `vitest`        | `boolean` | Enable Vitest ESLint rules for test files (default: `true`). Set to `false` when using `node:test` or another runner — this whitelists `node:test`'s `describe`/`it`/`test` in `no-floating-promises` via `allowForKnownSafeCalls`.                              |
