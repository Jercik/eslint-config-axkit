...
| `vitest`        | `boolean` | Enable Vitest ESLint rules for test files (default: `true`). Set to `false` when not using Vitest — whitelists `node:test`'s promise-returning functions in `no-floating-promises` via `allowForKnownSafeCalls`.                                                 |
