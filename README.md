...
| `vitest`        | `boolean` | Enable Vitest ESLint rules for test files (default: `true`). Set to `false` when not using Vitest — disables `@typescript-eslint/no-floating-promises` in test files since `node:test`'s functions return promises not meant to be awaited.                      |
